...
/**
 * The Mult Class represents an expression of multiple operator.
 *
 * @author Amit Twito
 */
...
*/
    ...
    public Expression simplify() {
        ...
        if (canParseDouble(simpleExp1.toString())) {
            //If the first one is 1, return the second : 1*X = X
            if (parseDouble(simpleExp1.toString()) == 1) {
                return simpleExp2;
            }
            // If it is zero, return the Num 0 : X*0 = 0
            if (parseDouble(simpleExp1.toString()) == 0) {
                return new Num(0);
            }
        }
        if (canParseDouble(simpleExp2.toString())) {
            //If the second one is 1, return the first : X*1 = X
