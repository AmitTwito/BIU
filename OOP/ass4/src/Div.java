...
/**
 * The Div Class represents an expression of divide operator.
 *
 * @author Amit Twito
 */
public class Div extends BinaryExpression implements Expression {
    ...
    public Expression simplify() {
        ...
        //If both arugments are equal - return the Num 1.
        ...
        //if the numerator is 0 - return the number 0.
    ...
    public Expression advancedSimplify() {
        ...
        if (advSimpleEx1 instanceof Sin
            ...
            if (sin.getFirstArgument().toString().equals(cos.getFirstArgument().toString())) {
                return new Tan(sin.getFirstArgument().simplify()).advancedSimplify();
            }
        }
        if (advSimpleEx1 instanceof Cos
            ...
            if (sin.getFirstArgument().toString().equals(cos.getFirstArgument().toString())) {
                return new Pow(new Tan(cos.getFirstArgument().simplify()), -1).advancedSimplify();
            }
        }
        if (advSimpleEx1 instanceof Mult) {
            Mult firstMult = (Mult) advSimpleEx1;
            //X/NUm = 1/Nnm * X
            if (advSimpleEx2 instanceof Num) {
                return new Mult(new Div(1, advSimpleEx2), firstMult).advancedSimplify();
            }
            if (advSimpleEx2 instanceof Mult) {
                Mult secondMult = (Mult) advSimpleEx2;
                if (firstMult.getFirstArgument().toString().equals(firstMult.getFirstArgument())) {
                    return new Div(firstMult.getSecondArgument(),
                            secondMult.getSecondArgument()).advancedSimplify();
                }

            }
        }

        return new Div(advSimpleEx1.simplify(), advSimpleEx2.simplify());
    }
}
